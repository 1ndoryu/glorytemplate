...
- ✅ A-C2 (avance): extraídas validación (`CalendarReglasValidator`), slots (`CalendarSlotsBuilder`), demanda (`CalendarDemandCalculator`), persistencia (`CalendarPersistenceService`), avisos (`CalendarCoverageNoticeBuilder`), distribución (`CalendarSubjectDistributor`), carga de datos (`CalendarDataLoader`), configuración/timezone (`CalendarEngineConfigProvider`), conflictos (`CalendarConflictDetector`) y contexto semanal (`CalendarWeekContextBuilder`).
...
- **Estado:** [~] En progreso (parcial aplicado)
...
- **Estado:** [~] En progreso — extraídas validación, slots, demanda, persistencia, avisos, distribución, carga de datos, configuración/timezone, conflictos y contexto semanal (`CalendarReglasValidator`, `CalendarSlotsBuilder`, `CalendarDemandCalculator`, `CalendarPersistenceService`, `CalendarCoverageNoticeBuilder`, `CalendarSubjectDistributor`, `CalendarDataLoader`, `CalendarEngineConfigProvider`, `CalendarConflictDetector`, `CalendarWeekContextBuilder`); `CalendarEngine` está en 311 líneas y queda pendiente recorte final para ≤300.

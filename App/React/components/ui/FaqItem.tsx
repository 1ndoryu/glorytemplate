...
export function FaqItem({question, answer}: FaqItemProps) {
    ...
    return (
        <div className="border-b border-primary">
            <button onClick={() => setIsOpen(!isOpen)} className="flex w-full items-center justify-between py-4 text-left text-sm font-medium transition-colors hover:text-black text-primary">
                {question}
                <ChevronDown className={`h-4 w-4 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''} text-subtle`} />
            </button>

...
export const TarjetaSample = ({sample, activa = false, reproduciendo = false, progreso = 0, onPlay, onPause, onSeek, onLike, onDescargar, onMenu, onClickCreador, className = ''}: TarjetaSampleProps): JSX.Element => {
    ...
    const clases = ['tarjetaSample', activa ? 'tarjetaSampleActiva' : '', className].filter(Boolean).join(' ');

    return (
        <div className={clases} onContextMenu={manejarMenu}>
            {/* Play/Pause */}
            <button className={`tarjetaPlayBtn ${activa && reproduciendo ? 'tarjetaPlayBtnActivo' : ''}`} onClick={manejarPlayPause} type="button" aria-label={reproduciendo && activa ? 'Pausar' : 'Reproducir'}>
            ...
            <div className="tarjetaContenido">
                ...
                <div className="tarjetaMeta">
                    <span className="tarjetaCreador" onClick={manejarClickCreador} role="link" tabIndex={0}>

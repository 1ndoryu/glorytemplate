...
const getTabContent = (): Record<string, TabContentItem> => ({
    whatsapp: {
        ...
        title: (
            <>
                Detecto, clasifico y <span className="text-subtle">doy seguimiento.</span>
            </>
        ),
    ...
    auto: {
        icon: <Database className="w-4 h-4 text-subtle" />,
        label: 'Automatizacion',
        title: (
            <>
                Formularios al CRM, <span className="text-subtle">sin copiar y pegar.</span>
            </>
        ),
    ...
    integraciones: {
        icon: <Layers className="w-4 h-4 text-subtle" />,
        label: 'Integraciones',
        title: (
            <>
                Conecto con <span className="text-subtle">lo que ya usas.</span>
            </>
        ),
...
export function BentoGrid({activeTab, onTabChange}: BentoGridProps): JSX.Element {
    ...
    return (
        <section id="bento-grid" className="mx-auto w-full max-w-7xl">
            <div id="bento-grid-container" className="grid grid-cols-4 lg:grid-cols-8 border rounded-md overflow-hidden shadow-sm border-primary bg-surface">
                {/* Integrations Bar */}
                <div id="bento-integrations-bar" className="col-span-4 lg:col-span-8 border-b bg-surface border-primary">
                    <div id="bento-integrations-grid" className="grid grid-cols-2 md:grid-cols-4 border-subtle">
                        {integrations.map((logo, idx) => (
                            <div key={idx} id={`bento-integration-${logo.toLowerCase()}`} className="h-24 flex items-center justify-center grayscale opacity-40 hover:grayscale-0 hover:opacity-80 transition-all cursor-default">
                                <span className="font-semibold tracking-tight text-lg font-mono text-primary">{logo}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* Bottom Content */}
                <div id="bento-bottom-content" className="col-span-4 lg:col-span-8 bg-surface">
                    <div id="bento-bottom-grid" className="grid grid-cols-1 lg:grid-cols-12 min-h-[360px]">
                        {/* Dynamic Content */}
                        <div id="bento-dynamic-content" className="col-span-1 lg:col-span-9 p-8 md:p-10 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-primary bg-surface">
                            <div id="bento-content-wrapper" className="h-full flex flex-col justify-center gap-4">
                                <div id="bento-icon-label" className="flex items-center gap-2">
                                    {currentContent.icon}
                                    <span className="text-xs font-mono uppercase text-subtle">{currentContent.label}</span>
                                </div>
                                <h3 id="bento-content-title" className="text-2xl md:text-3xl font-medium tracking-tighter text-primary">
                                    {currentContent.title}
                                </h3>
                                <p id="bento-content-description" className="leading-relaxed text-[15px] max-w-2xl text-muted">
                                    {currentContent.description}
                                </p>
                            </div>

                            {/* Tabs */}
                            <div id="bento-tabs-container" className="flex items-center gap-4 mt-8 pt-8 border-t border-subtle">
                                <span id="bento-tabs-label" className="text-sm font-medium text-muted">
                                    How Guillermo helps
                                </span>
                                <div id="bento-tabs-wrapper" className="flex p-1 rounded-md border bg-[var(--color-bg-tertiary)] border-primary">
                                    {tabs.map(tab => (
                                        <button
                                            key={tab.id}
                                            id={`bento-tab-${tab.id}`}
                        ...
                        {/* Sidebar CTA */}
                        <div id="bento-sidebar-cta" className="col-span-1 lg:col-span-3 p-8 flex flex-col justify-center bg-primary">
                            <h4 id="bento-cta-title" className="font-semibold mb-3 text-sm text-primary">

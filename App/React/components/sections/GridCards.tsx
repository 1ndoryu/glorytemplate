...
export function GridCards({title, subtitle, cards}: GridCardsProps) {
    return (
        <section className="mx-auto w-full max-w-7xl pb-24">
            <div className="mb-12">
                <h2 className="text-2xl md:text-3xl font-medium tracking-tight text-primary">{title}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-10">
                {cards.map((card, i) => (
                    <div key={i} className="flex gap-4 items-start group">
                        <div className="h-12 w-12 rounded-lg border flex items-center justify-center flex-none transition-colors bg-[var(--color-bg-secondary)] border-primary text-muted">
                            <card.icon className="w-5 h-5" />
                        </div>
                        <div>
                            <h3 className="text-[15px] font-medium text-primary">{card.title}</h3>

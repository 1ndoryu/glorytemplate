...
export function ProfileHero() {
    ...
    return (
        <section className="mx-auto w-full max-w-5xl">
            <div className="bg-white border border-[#e5e5e0] rounded-2xl overflow-hidden shadow-sm flex flex-col md:flex-row">
                {/* Left: Photo Area */}
                <div className="w-full md:w-2/5 bg-[#f0efeb] relative min-h-[300px] md:min-h-full">
                    ...
                    <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-sm p-3 rounded-lg border border-white/20 shadow-lg">
                        <div className="flex items-center gap-2 text-xs font-medium text-[#292524]">
                            <MapPin className="w-3 h-3 text-[#79716b]" />
                            {hero.location}
                        </div>
                    </div>
                </div>
                ...
                <div className="w-full md:w-3/5 p-8 md:p-12 flex flex-col justify-center">
                    <div className="flex items-center gap-2 mb-6">
                        <Badge className="bg-[#292524] text-white border-transparent">{hero.badges[0]}</Badge>
                        <Badge>{hero.badges[1]}</Badge>
                    </div>

                    <h1 className="text-3xl md:text-4xl font-semibold tracking-tight text-[#292524] mb-6">{hero.title}</h1>

                    <p className="text-[#79716b] leading-relaxed mb-8 font-light text-lg">{hero.description}</p>

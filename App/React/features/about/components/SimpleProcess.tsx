...
export function SimpleProcess() {
    ...
    return (
        <section id="simple-process" className="mx-auto w-full max-w-5xl border-t border-primary pt-16">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                <div className="md:w-1/3">
                    <h2 className="text-2xl font-heading font-semibold text-primary mb-2">{process.title}</h2>
                ...
                <div className="md:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {process.steps.map((item: {step: string; title: string; desc: string}, i: number) => (
                        <div key={i} className="flex gap-4">
                            <div className="text-2xl font-bold text-stone-200 font-mono">{item.step}</div>
                            <div>

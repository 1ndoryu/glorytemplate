...
- **C382 Fix 5 bugs sync** (1) **Historial reaparece**: race condition multi-ventana MPA. `limpiarHistorialSamples()` emite evento Tauri `limpiar-historial-samples`; ambas ventanas escuchan y limpian in-memory. (2) **Duplicados servidor**: `rutasEnVuelo` Set como guard de ruta paralelo en `subirArchivo` — cubre caso hash=null (OneDrive/FS lock). (3) **Rename carpeta**: `renombrarColeccionEnServidor` ahora encola a offline queue cuando offline o error transitorio (500/429). Callback en syncWatcherSetup elimina double-write (solo actualiza tracking local si server falla). (4) **Imágenes nunca aparecen**: `sync.tsx` ahora configura `window.__KAMPLES_CONFIG__` con serverUrl derivado de GLORY_CONTEXT — `obtenerOrigenServidorSync()` ya resuelve URLs relativas. `persistir()` hace read-merge-write de imagenUrl cross-window para evitar que sync panel sobreescriba datos de main window. Throttle recarga Store reducido a 2s. (5) **Falsos duplicados**: hash check en `encolarArchivoInterno` ahora verifica tracking — si archivo fue purgado (eliminado del servidor), evicta hash stale y permite re-upload. Eliminado fallback `buscarArchivoPorNombre` en dedup pre-upload (demasiado amplio: kick.wav en colección A bloqueaba kick.wav en B). `forzarResync` ahora limpia `hashesConocidos` via `limpiarHashesConocidos()`. `renombrar_coleccion` agregado a TipoOperacion. main.tsx: añadidos exports faltantes (recargarHistorialDesdeStore, rehidratarImagenesPendientesSync).
- **C384 Fix uploads fantasma + move sin colección** (1) **Uploads fantasma al reiniciar**: `inicializarUploadQueue` ahora verifica existencia física de archivos con `exists()` en restauración — items cuyo archivo fue eliminado/movido se purgan del store. `subirArchivo` en file-not-found salta reintentos (`intentos = MAX_REINTENTOS`) para que no quede como error persistente. (2) **Move a carpeta no crea colección**: `subirArchivo` ahora llama `crearColeccionDesdeLocal` (idempotente: check local + in-flight Map + server GET + 409) cuando `coleccionIdResuelta` es null y hay carpeta. `manejarMoveLocal` (v1 y v2) también asegura colección destino antes de `moverSampleEnServidor`. Resuelve C381 donde se eliminó creación durante upload pero no había reconciliación real.
- **C384b Fix ciclo infinito reintentos** `subirArchivo` retornaba `false` cuando archivo no existe → item quedaba 'error'. `ejecutarSync` (sync panel) resetea TODOS los 'error' → 'pendiente'+'intentos=0', rehaciendo el ciclo infinito. Fix: retornar `true` (→ 'completado', descarte permanente) + historial entries. Listener `reintentar-errores-upload` solo resetea `intentos < MAX_REINTENTOS`. Filter al cargar Store excluye `error+intentos>=MAX_REINTENTOS` para compatibilidad con items viejos.
- **C385 Escaneo local en "Sincronizar ahora"** Root cause: `ejecutarSync` solo descargaba servidor→local y reintentaba errores — nunca escaneaba la carpeta local para subir archivos nuevos. El watcher solo detecta eventos de cambio en tiempo real; archivos copiados con app cerrada o preexistentes al iniciar nunca se subían. Fix: `escanearCarpetaYEncolar()` en `syncWatcherSetup.ts` (scan `readDir` 2 niveles, skip `.papelera`/`sin colección`/`CARPETAS_SISTEMA_SYNC`, dedup via tracking v2+v1, llama `encolarArchivo`). Se invoca en startup (async, no bloquea) + listener Tauri `'escanear-subidas-local'`. `ejecutarSync` emite ese evento → ventana principal (que tiene la upload queue real) ejecuta el scan.
  - [Lección]: MPA Tauri — sync panel y main window son procesos JS separados. Operaciones de queue DEBEN ejecutarse en main window vía evento Tauri. sync panel solo puede escribir al Store directamente + emitir eventos para que main window recargue.

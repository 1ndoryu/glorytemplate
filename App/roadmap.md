...
- [EnlaceNavegacion]: Componente reutilizable `components/ui/EnlaceNavegacion.tsx` para enlaces SPA con soporte de apertura en nueva pestaña. Left-click → SPA `navegar()`. Middle-click/Ctrl+click → browser abre en nueva pestaña (comportamiento nativo de `<a href>`). Reemplaza patrón `<div onClick>` y `<BotonBase onClick>` para navegación. TarjetaColeccion, TarjetaPublicacion (autor+fecha), EnlaceCreador und FilaColecciones migrados. Imágenes de posts usan `<a href={url} target="_blank">` para abrir imagen en nueva pestaña con middle-click. CSS selectores actualizados: `button.imagenClickable` → `.imagenClickable`, `.tarjetaPubAutorBloque button` → `.tarjetaPubAutorBloque a`.

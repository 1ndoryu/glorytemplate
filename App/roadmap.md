...
- [Drag Nativo]: `@crabnebula/tauri-plugin-drag` v2.1.0 = Rust crate `tauri-plugin-drag = "2.1.0"`. Capability: `drag:default` + `drag:allow-start-drag`. Plugin: `.plugin(tauri_plugin_drag::init())`. JS: `startDrag({ item: [path] })`. Browser drag events (onDragStart) NO lanzan drag nativo de archivos — son sistemas separados. Usar `startDrag()` de Tauri async, no browser `dataTransfer`.

...
- [Sync UI]: syncStore.ts en `App/React/stores/` (puro Zustand, sin deps Tauri) accesible desde ambos builds. Hook usa dynamic import con `@vite-ignore` y string concatenation opaca para syncService. Componente desktop-only gated por `window.__KAMPLES_DESKTOP__` en TopBar.

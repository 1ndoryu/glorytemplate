...
- **Sentinel Sprint 5:** 5 nuevas reglas: `componente-artesanal` (detecta menus/modales artesanales vs MenuContextual/Modal), `fallo-sin-feedback` (catch sin toast), `update-optimista-sin-rollback` (set() sin rollback en catch), `fetch-sin-timeout` (fetch sin AbortController), `non-null-assertion-excesivo` (5+ `!` en archivo). Total: 48 reglas activas.

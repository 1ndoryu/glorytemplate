...
340. ✅ [AG-DRG] C339-fix2: Fix drag nativo OS-level — Bug raíz: `startDrag({ item, icon })` requiere parámetro `icon` obligatorio (ruta a imagen preview del drag). Se pasaba solo `item` → invoke Rust fallaba por `image` faltante. Segundo bug: `onMouseDown` conflictaba con clicks (play/pause). Fix: (1) `obtenerIconoDrag()` resuelve `resolveResource('icons/32x32.png')` + fallback PNG 1x1 transparente en temp, cache en memoria. (2) `startDrag()` ahora recibe `{ item: [path], icon: iconoDrag }`. (3) Drag nativo movido de `onMouseDown` a `onDragStart` — solo dispara con gesto de arrastre real, no clicks. `e.preventDefault()` cancela drag browser, luego `startDrag()` de Tauri toma control del mouse. (4) `manejarMouseDown` eliminado completamente (conflicto click/drag resuelto). (5) `icons/32x32.png` añadido a `bundle.resources` en tauri.conf.json. Archivos: audioLocalService.ts, useTarjetaSample.ts, TarjetaSample.tsx, tauri.conf.json.
---

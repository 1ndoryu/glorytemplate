...
#### 4.9.1 Variables de diseño existentes a reutilizar

Del sistema actual (`styles/dashboard/variables.css`):

| Variable | Valor | Uso en sidebar |
|----------|-------|----------------|
| `--dashboard-fondoPrincipal` | `#090909` | Fondo del sidebar |
| `--dashboard-fondoSecundario` | `#0c0c0c` (oscuro) | Fondo de item activo |
| `--dashboard-fondoHover` | `#0c0c0c` | Hover de items |
| `--dashboard-bordePrincipal` | `#1a1a1a` | Borde derecho del sidebar |
| `--dashboard-bordeSutil` | `#222` | Separadores entre items |
| `--dashboard-textoActivo` | `#eaeaea` | Icono/tooltip activo |
| `--dashboard-textoSecundario` | `#888` | Icono inactivo |
| `--dashboard-textoApagado` | `#666` | Tooltip texto secundario |
| `--dashboard-acento` | `#4a665b` | Indicador de panel activo (borde izq) |
| `--dashboard-acentoHover` | `#3d5a4e` | Hover indicador |
| `--dashboard-radioMd` | `0` / `4px` (según tema) | Bordes de items |
| `--dashboard-radioMedio` | `8px` | Bordes del tooltip |
| `--dashboard-radioCirculo` | `50%` | Avatares/dots |
| `--dashboard-radioPildora` | `9999px` | Badges |
| `--dashboard-espacioXs` | `0.25rem` (4px) | Gap entre iconos |
| `--dashboard-espacioSm` | `0.5rem` (8px) | Padding items |
| `--dashboard-espacioLg` | `1rem` (16px) | Padding sidebar |
| `--dashboard-espacioXl` | `1.5rem` (24px) | Espacio entre sidebar y contenido |
| `--dashboard-transicionRapida` | `0.15s ease` | Transiciones de hover/click |
| `--dashboard-transicionNormal` | `0.2s ease` | Transiciones de panel |
| `--dashboard-fuenteMonoespaciada` | `ui-monospace, ...` | Tipografía |
| `--dashboard-tamanoPequeno` | `11px` | Tooltip texto |
| `--dashboard-tamanoBase` | `13px` | Texto normal |
| `--dashboard-panelHeaderAltura` | `42px` | Altura header panel |
| `--dashboard-superposicionClara` | `rgba(125,125,125,0.05)` | Overlay sutil |
| `--dashboard-superposicionSutil` | `rgba(125,125,125,0.1)` | Overlay hover |

#### 4.9.2 Archivos CSS a crear

**Archivo 1:** `styles/dashboard/componentes/sidebarMenu.css`
...
.dashboardSidebarLayout {
    display: flex;
    flex-direction: row;
    height: calc(100vh - 120px); /* Resta header + padding */
    min-height: 0;
    gap: 0;
}
...
.sidebarMenu {
    ...
    width: 56px;                    /* Ancho fijo, angosto */
    min-width: 56px;
    flex-shrink: 0;
    background-color: var(--dashboard-fondoPrincipal);
    border-right: 1px solid var(--dashboard-bordePrincipal);
    padding: var(--dashboard-espacioSm) 0;
    gap: var(--dashboard-espacioXs);
    overflow-y: auto;
    overflow-x: hidden;
}
...
.sidebarMenuIcono {
    ...
    padding: var(--dashboard-espacioSm);
    background: none;
    border: none;
    cursor: pointer;
    color: var(--dashboard-textoSecundario);
    transition: color var(--dashboard-transicionRapida),
                background-color var(--dashboard-transicionRapida);
}

.sidebarMenuIcono:hover {
    color: var(--dashboard-textoActivo);
    background-color: var(--dashboard-fondoHover);
}

/* Estado activo — indicador izquierdo + highlight */
.sidebarMenuIcono--activo {
    color: var(--dashboard-textoActivo);
    background-color: var(--dashboard-fondoSecundario);
}

.sidebarMenuIcono--activo::before {
    content: '';
    position: absolute;
    left: 0;
    top: 25%;
    height: 50%;
    width: 3px;
    background-color: var(--dashboard-acento);
    border-radius: 0 var(--dashboard-radioXs) var(--dashboard-radioXs) 0;
}
...
.sidebarMenuTooltip {
    ...
    background-color: var(--dashboard-fondoTerciario);
    color: var(--dashboard-textoActivo);
    font-family: var(--dashboard-fuenteMonoespaciada);
    font-size: var(--dashboard-tamanoPequeno);
    padding: var(--dashboard-espacioXs) var(--dashboard-espacioSm);
    border-radius: var(--dashboard-radioMedio);
    border: 1px solid var(--dashboard-bordeSutil);
    white-space: nowrap;
    pointer-events: none;
    opacity: 0;
    transition: opacity var(--dashboard-transicionRapida);
    z-index: 1000;
}

.sidebarMenuIcono:hover .sidebarMenuTooltip {
    opacity: 1;
}

/* Separador visual entre grupos de iconos */
.sidebarMenuSeparador {
    height: 1px;
    background-color: var(--dashboard-bordeSutil);
    margin: var(--dashboard-espacioSm) var(--dashboard-espacioSm);
    opacity: 0.5;
}
...
.dashboardSidebarContenido {
    ...
    overflow-y: auto;
    overflow-x: hidden;
    padding: var(--dashboard-espacioLg);
}
...
.dashboardSidebarPanel {
    ...
    transition: opacity var(--dashboard-transicionNormal),
                transform var(--dashboard-transicionNormal);
}

.dashboardSidebarPanel--entrando {
    opacity: 0;
    transform: translateY(4px);
}
```

**Archivo 2:** `styles/dashboard/componentes/dashboardPanelView.css`

```css
/* Contenedor del panel individual en modo sidebar */
.dashboardPanelView {
    display: flex;
    flex-direction: column;
    width: 100%;
...
/* Wrapper del contenido — hereda estilos de panel existentes */
.dashboardPanelView .panelDashboard {
    height: 100%;
    display: flex;
    flex-direction: column;
}

/* Placeholder cuando no hay panel seleccionado */
.dashboardPanelVacio {
    display: flex;
    align-items: center;
    justify-content: center;
    ...
    font-family: var(--dashboard-fuenteMonoespaciada);
    font-size: var(--dashboard-tamanoBase);
}
```

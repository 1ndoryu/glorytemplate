...
- [C802c/UploadContexto]: Para vincular un sample a una entidad al subir, pasar el ID por `crearModalStore` (no por props). `useCrearContenido` lee `getState().relacionSampleoId` en el submit. SamplesUploadController lee `relacion_sampleo_id` del body (param GET/POST libre, sin declarar en args). Migración v035 agrega FK nullable en samples.
